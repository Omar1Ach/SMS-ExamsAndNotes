...
export default function StagiaireListPage() {
  ...
  useEffect(() => {
    if (id) {
      const fetchStagiaires = async () => {
        try {
          const response = await axios.get(`https://localhost:7263/api/ExamResult/id?id=${id}`);
  ...
  const handleEdit = (stagiaireId) => {
    ...
    const stagiaireToEdit = stagiaires.find((stagiaire) => stagiaire.stagiaireId === stagiaireId);
          ...
          <table className="min-w-full bg-white border border-gray-300 rounded-lg">
            <thead className="bg-gray-200 text-gray-800">
              <tr className="bg-blue-100">
                <th className="border border-gray-300 px-6 py-3 text-center font-bold bg-blue-200">Stagiaire</th>
            ...
            <tbody>
              {stagiaires.map((stagiaire, index) => (
                <tr key={stagiaire.stagiaireId} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                  <td className="border border-gray-300 px-6 py-3 text-sm text-gray-700">
                    {editId === stagiaire.stagiaireId ? (
                      <input
                        type="text"
                        value={editedStagiaire.stagiaireId || ''}
                        onChange={(e) => handleInputChange(e, 'stagiaireId')}
                      ...
                      <div className="text-center">
                        {editId === stagiaire.stagiaireId ? (
                          <input
                            ...
                            onChange={(e) => handleInputChange(e, 'theoreticalNote')}
                      ...
                      <div className="text-center">
                        {editId === stagiaire.stagiaireId ? (
                          <input
                            ...
                            onChange={(e) => handleInputChange(e, 'practicalNote')}
                            className="border border-gray-300 px-2 py-1 rounded w-24"
                          />
                        ) : (
              ...
              {stagiaires.length === 0 && (
                <tr>
                  <td colSpan="3" className="border border-gray-300 px-6 py-3 text-center text-sm text-gray-700">
